...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async getCosts(){
        await this.$db.collection('thalgoHome')
              .get()
              .then((doc) => {
                let localTables = []
      ...
      async addNewMonth(){
        if(this.$refs.newMonthForm.validate()){
          ...
          let baseCosts = (await this.$db
                .collection('baseThalgoHome')
                .doc('J91KwcHTmlWziXIc3zmW')
                .get()).data()
      ...
      async removeMonth(id){
        ...
        if(res){
          this.loading = true
          await this.$db.collection('thalgoHome').doc(id).delete()
          await this.getCosts()
          this.$notify({
            group: 'foo',
            type: 'success',
