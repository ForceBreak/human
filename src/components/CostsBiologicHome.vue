...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async getCosts(){
        await this.$db.collection('biologicHome')
              .get()
              .then((doc) => {
                let localTables = []
      ...
      async addNewMonth(){
        if(this.$refs.newMonthForm.validate()){
          ...
          let baseCosts = (await this.$db
                .collection('baseBiologicHome')
                .doc('qVIDpIDxxdQZK4AnUJcP')
                .get()).data()
      ...
      async removeMonth(id){
        ...
        if(res){
          this.loading = true
          await this.$db.collection('biologicHome').doc(id).delete()
          await this.getCosts()
          this.$notify({
            group: 'foo',
            type: 'success',
